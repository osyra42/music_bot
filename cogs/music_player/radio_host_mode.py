...
@commands.slash_command(name="set_tone", description="Set the tone for the radio host commentary.", default_member_permissions=disnake.Permissions(administrator=True))
async def set_tone(self, inter: disnake.ApplicationCommandInteraction, tone: str):
    """Set the tone for the radio host commentary."""
    valid_tones = ["humorous", "formal"]
    if tone not in valid_tones:
        await inter.response.send_message(f"Invalid tone. Valid tones are: {', '.join(valid_tones)}")
        return

    self.tone = tone
    await inter.response.send_message(f"Tone set to {tone}.")

@commands.slash_command(name="set_news_categories", description="Set the news categories for the radio host commentary.", default_member_permissions=disnake.Permissions(administrator=True))
async def set_news_categories(self, inter: disnake.ApplicationCommandInteraction, categories: str):
    """Set the news categories for the radio host commentary."""
    self.news_categories = categories.split(",")
    await inter.response.send_message(f"News categories set to {', '.join(self.news_categories)}.")
